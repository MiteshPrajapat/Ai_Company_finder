...
class JobDetailsDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        # Contact HR with AI Button
        contact_hr_btn = QPushButton("✉️  Contact HR with AI")
        contact_hr_btn.setObjectName("btnPrimary")
        contact_hr_btn.setStyleSheet("background-color: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #0284C7, stop:1 #06B6D4); font-weight: 700;")
        contact_hr_btn.clicked.connect(self._open_contact_hr)
        action_row.addWidget(contact_hr_btn)

        action_row.addStretch()

        if self.job.job_url:
            open_btn = QPushButton("🌐  Open Primary Link")
            open_btn.setObjectName("btnAction")
    ...
    def _open_contact_hr(self):
        from ui.contact_hr_dialog import ContactHRDialog
        dlg = ContactHRDialog(self.job, self)
        dlg.exec()
