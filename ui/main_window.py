...
from ui.applications_widget import ApplicationsWidget
...
class MainWindow(QMainWindow):
    ...
    def _init_ui(self):
        ...
        # Page 4: Applications Tracker
        self.page_applications = ApplicationsWidget()
        self.pages.addWidget(self.page_applications)
    ...
    def _create_search_page(self) -> QWidget:
        ...
        self.results_widget.contact_hr_requested.connect(self._open_contact_hr_for_company)
    ...
    def _create_all_companies_page(self) -> QWidget:
        ...
        self.all_companies_results.contact_hr_requested.connect(self._open_contact_hr_for_company)
    ...
    def _on_nav_changed(self, row: int):
        ...
        if row == 1:
            ...
            self.page_applications.reload_applications()
        elif row == 5:
    ...
    def _filter_jobs_table(self):
        ...
        for row, job in enumerate(filtered):
            ...
            btn_contact.clicked.connect(lambda checked=False, j=job: self._open_contact_hr_modal(j))
            btn_layout.addWidget(btn_contact)

            btn_view = QPushButton("👁️ View")
            btn_view.setObjectName("btnAction")
            btn_view.setStyleSheet("font-size: 10px; padding: 2px 6px;")
            btn_view.setFixedHeight(24)
            btn_view.clicked.connect(lambda checked=False, j=job: self._open_job_modal(j))
            btn_layout.addWidget(btn_view)

            self.jobs_table.setCellWidget(row, 7, btn_box)
    ...
    def _open_contact_hr_modal(self, job: Job):
        dialog = ContactHRDialog(job=job, parent=self)
        dialog.exec()
        self.page_applications.reload_applications()

    def _open_contact_hr_for_company(self, company_id: int):
        company = self.companies_repo.get_by_id(company_id)
        if not company:
            QMessageBox.warning(self, "Company Not Found", "Selected company could not be found.")
            return

        jobs = self.jobs_repo.get_by_company(company_id)
        target_job = jobs[0] if jobs else None

        dialog = ContactHRDialog(job=target_job, company=company, parent=self)
        dialog.exec()
        self.page_applications.reload_applications()
