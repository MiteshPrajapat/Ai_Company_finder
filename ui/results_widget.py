...
class ResultsWidget(QWidget):
    ...
    contact_hr_requested = pyqtSignal(int)  # company_id
    ...
    def _init_ui(self):
        ...
        self.table.setHorizontalHeaderLabels([
            ...
            "Website", "Rating", "Web Status", "Jobs", "Emails", "Enrichment", "Action"
        ])
    ...
    def _render_row(self, row: int, comp: Company):
        ...
        # 11. Action Buttons Container (Quick Contact HR / View)
        ...
        btn_contact.clicked.connect(lambda checked=False, cid=comp.id: self.contact_hr_requested.emit(cid))
        btn_layout.addWidget(btn_contact)

        btn_view = QPushButton("👁️")
        btn_view.setObjectName("btnAction")
        btn_view.setStyleSheet("font-size: 10px; padding: 2px 4px;")
        btn_view.setFixedWidth(28)
        btn_view.setFixedHeight(24)
        btn_view.setToolTip("View Company Details")
        btn_view.clicked.connect(lambda checked=False, cid=comp.id: self.company_selected.emit(cid))
        btn_layout.addWidget(btn_view)

        self.table.setCellWidget(row, 11, btn_container)
    ...
    def _show_context_menu(self, pos):
        ...
        action_contact = menu.addAction("✉️  Contact HR / Send Resume (AI)")
        action_contact.triggered.connect(lambda: self.contact_hr_requested.emit(comp_id))
